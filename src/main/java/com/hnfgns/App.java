...
public class App {
  ...
  final static int POST_ALLOC_SLEEP = 10 * 1000; // 20 secs
  final static int POST_FREE_SLEEP = 10 * 1000; // 20 secs
  ...
  public static void main(String[] args)  throws Exception {
    ...
    for (;;) {
...

      {
        ...
        logger.info("De-allocating {} chunks from {}", Math.max(curUntil - freeFrom, 0), freeFrom);
        for (int i = freeFrom; i < curUntil; i++) {
          free(buffers[i]);
        }
